...
def sobre(request):
    if request.user.is_authenticated: #mostra a pág home se o usuario tiver logado/autenticado
        return render(request, 'usuarios/sobre.html') #renderizando o template
    else:
        return render(request, 'usuarios/login.html')

def contato(request):
    if request.user.is_authenticated: #mostra a pág home se o usuario tiver logado/autenticado
        return render(request, 'usuarios/contato.html') #renderizando o template
    else:
        return render(request, 'usuarios/login.html')
...
def lancar(request):
    if request.method == "GET":
        ...
        if nota_verificada:
            return HttpResponse("Despesa já possui dados cadastrados!")
        else:
            nota.save()
            return render(request, 'usuarios/home.html')
...
def editar(request, pk):
    if request.method == "POST":
        if request.user.is_authenticated:
            ...
            Nota.objects.filter(pk=pk).update(nome_aluno = nome_aluno, disciplina = disciplina, nota_atividades = nota_atividades, nota_trabalho = nota_trabalho, nota_prova = nota_prova)
            return HttpResponseRedirect(reverse('alterar'))
        else:
            return render(request, 'usuarios/login.html')
...
def visualizar(request):
    if request.method == "GET":
        ...
        disciplina = request.POST.get('disciplina')
        if disciplina == "Todas as despesas":
            lista_notas = Nota.objects.all()
            dicionario_notas = {'lista_notas':lista_notas}
            return render(request, 'usuarios/visualizar.html', dicionario_notas)
        else:
